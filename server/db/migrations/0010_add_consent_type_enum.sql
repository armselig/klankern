...
-- This will fail if there are invalid or NULL consent_type values
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM "user_consents"
        WHERE "consent_type" IS NULL
           OR "consent_type" NOT IN ('marketing', 'analytics', 'data_processing', 'third_party_sharing')
    ) THEN
        RAISE EXCEPTION 'Invalid or NULL consent_type values found. All values must be non-NULL and one of: marketing, analytics, data_processing, third_party_sharing';
    END IF;
...
-- At this point we know all values are non-NULL and valid
UPDATE "user_consents" 
SET "consent_type_new" = "consent_type"::"consent_type";--> statement-breakpoint
